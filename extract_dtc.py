...
dtc_regex = re.compile('([A-Za-z][0-9]{4})')
...
for dtc_node in soup.find_all(has_regex(dtc_regex)):
    code = dtc_regex.search(dtc_node.string).group(0)
    problem_root = dtc_node.find_next_sibling(has_regex(dtc_regex))
    if problem_root:
        ...
        if cause_list_root:
            ...
            cur.execute('INSERT OR IGNORE INTO nd_dtc (code) VALUES (?)', (code,))
