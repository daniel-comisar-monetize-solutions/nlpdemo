...
fig_regex = re.compile('^Fig\. \d+: (Identifying )?((?:(?!Courtesy).)*)')
number_regex = re.compile('\s*(\(\d+\))\s*')
start_num_regex = re.compile('^\s*(\(\d+\))\s*')
...
for num_node in soup.find_all(has_regex(number_regex)):
    ...
    text = start_num_regex.sub('', num_node.text).strip()
    if blank_regex.search(text):
        continue

    id_node = num_node
    while not id_node.text or not fig_regex.search(id_node.text):
        ...
        id_node = prev_node
    (is_id, name) = fig_regex.search(id_node.text).groups()
    if not is_id:
        continue
    name = name.strip()
